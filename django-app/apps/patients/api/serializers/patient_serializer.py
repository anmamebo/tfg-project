    ...
    if user_data: # Si hay datos de usuario
      ...
      if user_serializer.is_valid(): # Si el serializador es valido
        ...
        if address_data: # Si hay datos de direccion
          address_serializer = AddressSerializer(data=address_data) # Se crea el serializador de direccion
          if address_serializer.is_valid(): # Si el serializador es valido
            address = address_serializer.save() # Se crea la direccion
            
            patient = Patient.objects.create(user=user, address=address, **validated_data) # Se crea el paciente
          ...
          patient = Patient.objects.create(user=user, **validated_data) # Se crea el paciente
