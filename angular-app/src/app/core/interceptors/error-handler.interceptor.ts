    ...
    return next.handle(request).pipe(
      catchError((error: HttpErrorResponse) => {
        ...
        const handleNestedErrors = (errors: any) => {
          if (typeof errors === 'object') { // Si es un objeto
            Object.keys(errors).forEach((key: any) => { // Recorremos las llaves
              const errorValue = errors[key]; // Obtenemos el valor del error
              
              if (Array.isArray(errorValue)) { // Si es un arreglo
                errorValue.forEach((message: any) => { // Recorremos los mensajes
                  errorMessage += '\n' + message; 
                });
              } else if (typeof errorValue === 'object') { // Si es un objeto
