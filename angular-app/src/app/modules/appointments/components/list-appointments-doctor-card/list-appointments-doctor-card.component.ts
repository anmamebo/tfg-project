...
export class ListAppointmentsDoctorCardComponent extends GenericListCardComponent {
  ...
  public override getItems(page: number, searchTerm?: string): void {
    ...
    this.appointmentService.getAppointmentsByDoctor(page, this.entityData.numResults, this.entityData.search.search, true, this.filterState, this.sort.column, this.sort.order).subscribe({
      next: (response: any) => {
